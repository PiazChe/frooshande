...
.factory('Source', function($rootScope) {
  ...
  return {
    initial : function(){
      var temp = window.localStorage['Sources'];
      if(temp == null)
        source = []
      else
    ...
    search : function(item){
      for (var i = 0; i < source.length; i++) {
        if (source[i].id == parseInt(item.id)) {
          return true;
        }
      }
      return false;
    ...
    add : function(item){
      item.prevRemain = item.remain;
      source.push( item);
      localStorage.setItem('Sources', JSON.stringify(source));
      //window.localStorage['Sources'] = source;
    },
    all: function() {
      return source;
    },
    modify :function(item)
    {
      for (var i = 0; i < source.length; i++) {
        if (source[i].id == parseInt(item.id)) {
          source[i].remain = item.remain;
          source[i].name= item.name;
          source[i].size= item.size;
          source[i].cost= item.cost;
          source[i].company = item.company;
          $rootScope.$emit('updateSource',[]);
        }
      }
      //window.localStorage['Sources'] = source;
      localStorage.setItem('Sources', JSON.stringify(source));
    ...
    remove: function(item) {
      source.splice(indexOf(item.id), 1);
      localStorage.setItem('Sources', JSON.stringify(source));
    },
    get: function(chatId) {
      for (var i = 0; i < source.length; i++) {
        if (source[i].id === parseInt(chatId)) {
          return source[i];
        }
      }
      return null;
  ...
  .factory('dateFactory',function()
  {
    ...
    /*
     input : dd/mm/yyyy
     output : set day indexes ==>day thet start week has index = 0
     (in shamsi : shanbe = 0 , 1shanbe = 1 ,... , jome = 6)
     (in georgian : sunday = 0,monday = 1 , ... , saturday = 6)
     */
    factory.getDayIndex = function(date)
    {
      var calendarType = window.localStorage['calendarType'];
      if(calendarType == 'shamsi')
      {
        var split = date.split('/');
        var d = new Date(parseInt(split[2]),parseInt(split[0]) - 1,parseInt(split[1]));
        return (d.getDay() + 1)%7;
      }
      else {
        var split = date.split('/');
